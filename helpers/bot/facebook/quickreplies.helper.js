        ...
        floatingButtons.forEach((btn) => {
            switch (btn) {
                case FloatingButtons.Ship:
                    quick_replies.push(this.buildQuickReply('Sự kiện', constants.COMMAND_SUKIEN));
                    break;
                // case FloatingButtons.Promotion:
                //     quick_replies.push(this.buildQuickReply('Các cây cầu', constants.COMMAND_PROMOTION));
                //     break;
                // case FloatingButtons.Menu:
                //     quick_replies.push(this.buildQuickReply('Thiên nhiên', constants.COMMAND_MENU));
                //     break;
                case FloatingButtons.Reservation:
                    quick_replies.push(this.buildQuickReply('Điểm tham quan', constants.COMMAND_THAMQUAN));
                    break;
                case FloatingButtons.AboutUs:
                    quick_replies.push(this.buildQuickReply('Về Đà Nẵng', constants.COMMAND_ABOUTDN));
                    break;
                case FloatingButtons.ChatWithAdmin:
                    quick_replies.push(this.buildQuickReply('Chat with Admin', constants.COMMAND_CHAT_WITH_ADMIN));
                    break;
                case FloatingButtons.StopChatting:
                    quick_replies.push(this.buildQuickReply('Stop chatting', constants.COMMAND_STOP_CHATTING));
                    break;
                // case FloatingButtons.Reservation:
